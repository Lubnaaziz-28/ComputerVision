...
# ==============================================================================
"""Definition of 300 VGG-based MREFP-Net network.

1- This model is reformation of ssd and FPN network with two scheme
Multi-layered feature enrichment scheme and Cascaded Anchor Refinement
scheme

2- Two variants of the model are defined: the 300x300 and 512x512 models, the
latter obtaining a slightly better accuracy on Pascal VOC.

Usage:
    with slim.arg_scope(pssd_vgg.ssd_vgg()):
        outputs, end_points = pssd_vgg.pssd_vgg(inputs)
...
class SSDModel():
    ...
    def __init__(self):
        ...
        self.model_name = 'proposed_ssd' # Proposed SSD
    ...
    def __additional_ssd_block(self, end_points, net):
        ...
        blk_1 = end_points['block1']
        blk_1 = slim.conv2d(blk_1, 128, [1,1], scope='blk_1')

        # Block 2.(SHALLOW FEAUTRES)
        blk_2 = end_points['block2']
        blk_2 = slim.conv2d(blk_2, 128, [3,3], scope='blk_2')
        tf.nn.relu(blk_2, name=None)
        #net = slim.repeat(net, 2, slim.conv2d, 128, [3, 3], scope= 'conv2')


        #net = tensor_resize.tensor4_resize(net, [300,300])
        #blk_1 = tensor_resize.tensor4_resize(blk_1, [150,150])
        blk_1 = slim.max_pool2d(blk_1, [2, 2], scope='pool1_blk1')
        shallow_f = blk_2+  blk_1


        # Block 3.(MIDDEL LEVEL FEATURES)
        blk_3 = end_points['block3']
        tf.nn.relu(blk_3, name=None)
        blk_3 = slim.conv2d(blk_3, 256, [3,3], scope='blk_3')
        #blk_2_3 = tensor_resize.tensor4_resize(shallow_f, [75,75])
        blk_2_3 = slim.max_pool2d(shallow_f, [2, 2], scope='pool2_blk2')
        #net = tensor_resize.tensor4_resize(net, [300,300])
        
        middle_f =  slim.conv2d(blk_2_3, 256, [1,1], scope='blk_2_3') + blk_3



        # Block 4.(DEEP FEATURES)
        blk_4 = end_points['block4']
        tf.nn.relu(blk_4, name=None)
        blk_4 = slim.conv2d(blk_4, 512, [3,3], scope='blk_4')

        #middle_f = tensor_resize.tensor4_resize(middle_f, [38,38])
        middle_f = slim.max_pool2d(middle_f, [2, 2], scope='pool_middle')
        blk_3_4 = middle_f
        blk_3_4= slim.repeat(blk_3_4, 1, slim.conv2d, 512, [1,1],  scope='blk_3_4')
        deep_f = blk_3_4 + blk_4




        # Block 5. (Multi-Level feature fusion block sum/concatinate)
        #shallow_f = tensor_resize.tensor4_resize(shallow_f, [38,38])
       
        shallow_f = slim.max_pool2d(blk_2_3, [2, 2], scope='pool_shallow')
        blk_5 =tf.concat([ shallow_f, middle_f, deep_f], axis = 3, name='concat')
        #net = blk_2+ blk_3 + end_points['block4']

        #net = slim.repeat(net, 3, slim.conv2d, 64, [1, 1], scope='conv5')

        #net = blk_5
        blk_5 = slim.repeat(blk_5, 3, slim.conv2d, 512, [3, 3], scope='blk_5')
        #blk_5 = tensor_resize.tensor4_resize(blk_5, [19,19])
        blk_5 = slim.max_pool2d(blk_5, [2, 2], scope='pool_blk5')
        f_f = blk_5+ end_points['block5']
        net = f_f
        #net = slim.max_pool2d(net, [3, 3], stride=1, scope='pool5')

        # Additional SSD blocks.
        # Block 6: let's dilate the hell out of it!
        tf.nn.relu(net, name=None)
        net_relu = net 
        net = slim.max_pool2d(net, [2,2], stride=1, scope='pool6')
        net = slim.conv2d(net, 512, [3, 3], rate=6, scope='conv6')
        
        end_points['block6'] = net + net_relu 
        net = end_points['block6']
        #net = tensor_resize.tensor4_resize(net, [75,75])
        
        
        
        
        # Block 7: 1x1 conv. (chain parallel pooling)
        
        #net = slim.conv2d(net, 256, [1, 1], scope='conv7')
        net = slim.conv2d(net, 512, [3, 3], stride= 1, scope='conv7')
        end_points['block7'] = net + end_points['block6'] # till now sizeof block 7 19*19
        net = slim.max_pool2d(net, [2,2], stride=1, scope='pool7')
        #net = slim.conv2d(net, 128, [1,1], scope='conv7_1')
        net = custom_layers.pad2d(net, pad=(1, 1))
        net = slim.conv2d(net, 256, [3, 3], stride=1, scope='conv7_1', padding='VALID') # now the size 19*19
       
        
        blk_7 = slim.max_pool2d(end_points['block7'], [2, 2], stride= 1,  scope='pool_blk7')
        blk_7 = slim.conv2d(blk_7, 256, [1, 1],  scope='blk_7')
        
        
        end_points['block7'] = net + blk_7
        #net = end_points['block7']
